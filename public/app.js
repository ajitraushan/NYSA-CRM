...
function previewProposalTemplate(){const c=proposalConfigurationFromForm(),form=$('#proposal-template-form'),fields=c.buyerBooklet.propertyFields.map(f=>PROPOSAL_PROPERTY_FIELDS.find(x=>x[0]===f.code)?.[1]).filter(Boolean),organization=organizationVersions.find(x=>x.status==='active'),logo=organization?.logo?.downloadUrl,draftVersion=form.elements.id.value?form.elements.id.value.slice(0,8):'unsaved';const o=overlay(`<div class="modal proposal-preview"><button class="close-x">×</button><div class="proposal-draft-watermark">DRAFT SAMPLE</div><header class="proposal-preview-brand">${logo?`<img src="${esc(logo)}" alt="${esc(organization.displayName)} approved logo">`:'<div class="proposal-logo-missing">No active approved logo</div>'}<div><b>${esc(organization?.displayName||'NYSA Realty')}</b><small>${esc(form.elements.name.value||'Quick Buyer Booklet')} · Brand ${esc(organization?.brandVersion||form.elements.brandVersion.value||'not selected')}</small></div></header><h2>Draft template layout</h2><p class="tool-note">Draft reference ${esc(draftVersion)}. Sample values are not saved or customer-ready.${organization?' The active company profile and approved logo shown here will govern live proposals.':' Activate an approved NYSA company profile before approval.'}</p><h3>Your requirements</h3><p>Dubai Marina · Apartment · 2 bedrooms · AED 2.0m–2.5m · ${c.buyerBooklet.requireAvailabilityCheck?'Availability must be reconfirmed':'Availability check optional'}</p><h3>Matched shortlist (maximum ${c.buyerBooklet.maxProperties})</h3><article class="proposal-sample"><b>Sample Residence · Dubai Marina</b><p>${fields.join(' · ')}</p><small>Why it matches: preferred location and budget. Trade-off: sample text to be completed from verified inventory and agent review.</small></article><h3>Approved property media</h3><p>Up to ${c.buyerBooklet.maxMediaPerProperty} approved images per property.</p><h3>Indicative purchase timeline</h3>${c.buyerBooklet.timelineStages.map(x=>`<p><b>${esc(x.label)}</b><br><small>${esc(x.guidance)}</small></p>`).join('')}<h3>Next steps</h3><p>Confirm availability, arrange viewing and verify finance or readiness before commitment.</p><footer class="proposal-preview-footer">${esc(organization?.proposalFooter||'NYSA Realty | Private customer proposal')}</footer><div class="modal-actions proposal-sample-actions"><button class="btn btn-primary" id="proposal-sample-print" type="button">Print / Save draft PDF</button></div></div>`);$('#proposal-sample-print',o).addEventListener('click',()=>window.print());}
