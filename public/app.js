...
function previewProposalTemplate(){const c=proposalConfigurationFromForm(),form=$('#proposal-template-form'),organization=organizationVersions.find(x=>x.status==='active'),logo=organization?.logo?.downloadUrl,draftVersion=form.elements.id.value?form.elements.id.value.slice(0,8):'unsaved',sampleConditions=new Set(['always','ready_property','bank_finance']),icons={confirm_requirements:'✓',viewing_selection:'⌂',finance_valuation:'AED',contract_due_diligence:'✎',transfer_handover:'⌁',developer_milestones:'◇'},shortGuidance={confirm_requirements:'1–2 business days',viewing_selection:'Subject to access',finance_valuation:'Lender timeline',contract_due_diligence:'Subject to documents',transfer_handover:'Authority appointment',developer_milestones:'Developer programme'},timeline=c.buyerBooklet.timelineStages.filter(x=>sampleConditions.has(x.condition||'always'));const o=overlay(`<div class="modal proposal-preview"><button class="close-x">×</button><div class="proposal-draft-watermark">DRAFT SAMPLE</div><header class="proposal-preview-brand">${logo?`<img src="${esc(logo)}" alt="${esc(organization.displayName)} approved logo">`:'<div class="proposal-logo-missing">No active approved logo</div>'}<div><b>${esc(organization?.displayName||'NYSA Realty')}</b><small>${esc(form.elements.name.value||'Quick Buyer Booklet')}</small></div></header><div class="proposal-title-row"><div><span>PRIVATE BUYER PROPOSAL</span><h2>Your Dubai property shortlist</h2></div><small>Draft ${esc(draftVersion)}</small></div><section class="proposal-buyer"><div><small>PREPARED FOR</small><b>Aisha Rahman</b></div><div><small>CUSTOMER ADDRESS</small><b>Palm Jumeirah, Dubai, UAE</b></div><div><small>MOBILE</small><b>+971 50 123 4567</b></div><div><small>KYC SUMMARY</small><b>Verified · Emirates ID ***4821</b></div></section><section class="proposal-kyc"><div><span>✓</span><div><small>CUSTOMER KYC DETAILS</small><b>Emirates ID ending 4821</b></div></div><div><small>STATUS</small><b>Verified</b></div><div><small>EXPIRY</small><b>31 Dec 2028</b></div><p>Full identity numbers and document copies remain in restricted private records and are not printed in this proposal.</p></section><section><h3>Your requirements</h3><div class="proposal-chips"><span>Purchase</span><span>Dubai Marina</span><span>Apartment</span><span>2 bedrooms</span><span>AED 2.0m–2.5m</span><span>Bank finance</span></div></section><section><div class="proposal-section-head"><h3>Recommended match</h3><small>1 of up to ${c.buyerBooklet.maxProperties}</small></div><article class="proposal-property-card"><div class="proposal-media-grid"><div><span>PROPERTY IMAGE 1</span></div><div><span>PROPERTY IMAGE 2</span></div></div><div class="proposal-property-heading"><div><b>Marina View Residence</b><small>Dubai Marina · Ready property</small></div><strong>AED 2,350,000</strong></div><div class="proposal-facts"><div><small>BUILT-UP AREA</small><b>1,245 sq ft</b></div><div><small>DEVELOPER</small><b>Sample Developer</b></div><div><small>BED / BATH</small><b>2 / 3</b></div><div><small>PARKING</small><b>1 space</b></div><div><small>BUILDING AGE</small><b>4 years</b></div><div><small>AVAILABILITY</small><b>Reconfirm before viewing</b></div></div><div class="proposal-assessment"><div><small>WHY IT MATCHES</small><b>Preferred location, layout and budget</b></div><div><small>VALUE</small><b>Marina view, established community and practical layout</b></div><div><small>TRADE-OFF</small><b>Bank valuation and current availability remain subject to confirmation</b></div></div><p class="proposal-amenities"><b>Amenities:</b> Pool · Gym · Concierge · Covered parking · Marina access</p></article></section><section><h3>Indicative purchase journey</h3><div class="proposal-timeline">${timeline.map((x,i)=>`<div class="proposal-timeline-step"><span>${esc(icons[x.code]||String(i+1))}</span><b>${esc(x.label)}</b><small>${esc(shortGuidance[x.code]||'Timing varies')}</small></div>`).join('')}</div></section><section><h3>Next steps</h3><div class="proposal-next-steps"><div><span>1</span><b>Confirm shortlist</b></div><div><span>2</span><b>Arrange viewing</b></div><div><span>3</span><b>Verify finance readiness</b></div></div></section><p class="proposal-disclaimer">Sample information only. Availability, pricing and transaction timing must be reconfirmed before commitment.</p><footer class="proposal-preview-footer">${esc(organization?.proposalFooter||'NYSA Realty | Private customer proposal')}</footer><div class="modal-actions proposal-sample-actions"><button class="btn btn-primary" id="proposal-sample-print" type="button">Print / Save draft PDF</button></div></div>`);$('#proposal-sample-print',o).addEventListener('click',()=>window.print());}
