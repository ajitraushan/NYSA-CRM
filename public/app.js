...
async function openAssignmentQueue() {
  let leads,staff;try{[{leads},{staff}]=await Promise.all([api('/crm/assignment-queue'),api('/crm/staff')]);}catch(err){return toast(err.message);}const leader=['admin','manager','director'].includes(ME.jobRole)||ME.role==='admin';
  const o=overlay(`<div class="modal lead-modal"><button class="close-x">×</button><h2>${ME.jobRole==='director'?'Company Unassigned / Pending Assignment Queue':'Pending Assignment Queue'}</h2><p class="tool-note">SLA-breached unattended leads return to their routed team queue. Eligible agents claim on a first-successful-claim basis; every cycle remains audited.</p><div class="pipeline-table-wrap"><table><tr><th>Customer / opportunity</th><th>Routing reason</th><th>Current team</th><th>Status / cycle</th><th>SLA deadline</th><th>Waiting</th><th>Action</th></tr>${leads.map(l=>`<tr><td>${esc(l.contactName)}<small>${esc(l.title)}</small></td><td>${esc(l.routingReason||l.businessType||'Unmatched fallback')}</td><td>${esc(l.teamName||'Company Unassigned Queue')}</td><td>${esc(l.assignmentStatus.replace('_',' '))}<small>Cycle ${l.queueCycleNo}</small></td><td>${fmtDate(l.slaDeadline)}</td><td>${Math.floor(Number(l.queueWaitSeconds||0)/60)} minutes</td><td>${leader?`<select data-queue-agent="${l.id}"><option value="">Select eligible agent</option>${staff.filter(s=>!l.assignedTeamId||s.teamId===l.assignedTeamId).map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('')}</select> <button class="btn btn-primary btn-sm" data-assign-queue="${l.id}" data-team="${l.assignedTeamId||''}">Assign</button>`:l.assignedTeamId===ME.teamId?`<button class="btn btn-primary btn-sm" data-claim="${l.id}">Claim for myself</button>`:'Director assignment required'}</td></tr>`).join('')}</table>${leads.length?'':'<div class="empty">The assignment queue is clear.</div>'}</div></div>`);
...
function openListingForm(l = null) {
  ...
  <div class="modal">
    ...
    <form id="listing-form">
      <div class="form-grid">
        ...
        <div><label>Portal readiness</label><select name="portalStatus">${opts(['not_ready','ready','published','blocked'],l?.portalStatus||'not_ready')}</select></div>
  ...
  $('#listing-form', o).addEventListener('submit', async (e) => {
    ...
    for (const k of ['sizeSqft','price','referencePrice','downPaymentPercent','onHandoverPercent','postHandoverYears'])
      f[k] = f[k] === '' ? null : +f[k];
    for (const k of ['developer','bedrooms','paymentPlanType','paymentPlanNotes','handoverDate','contact','notes','availabilityConfirmedAt','verificationExpiresAt','permitNumber','permitExpiresAt'])
      if (f[k] === '') f[k] = null;
    try {
...
/* ============ ADMIN ============ */
let adminActiveSection='organization';
function setupAdminWorkspace(){
  ...
  const definitions=[['organization','Company profile'],['controlled_values','Controlled values'],['sla','Business hours & SLA'],['routing','Lead routing & queues'],['qualification','Lead qualification'],['fees','Regulatory & fee rules'],['proposals','Proposal designer'],['documents','Document templates'],['targets','Dashboard targets'],['teams','CRM teams'],['users','User management'],['users','User records'],['operations','Operations & audit']];
...
async function loadQualificationModels(){try{const{models}=await api('/admin/qualification-models');$('#qualification-model-table').innerHTML=`<table><tr><th>Lead Qualification Version</th><th>Factors / weight</th><th>Score bands</th><th>Status</th><th>Actions</th></tr>${models.map(m=>`<tr><td>${esc(m.name)} v${m.version}<small>${esc(m.modelCode)} · ${esc(m.purpose)}</small></td><td>${m.factors.length} factors · ${m.factors.reduce((s,x)=>s+Number(x.weight),0)}%</td><td>Cold 0–${m.thresholds.warmMin-0.01}<br>Warm ${m.thresholds.warmMin}–${m.thresholds.hotMin-0.01}<br>Hot ${m.thresholds.hotMin}–100</td><td>${esc(m.status)}</td><td><button class="btn btn-sm" data-test-model="${m.id}">Test score</button>${m.status==='draft'?` <button class="btn btn-sm" data-approve-model="${m.id}">Approve</button>`:''}${m.status==='approved'?` <button class="btn btn-primary btn-sm" data-activate-model="${m.id}">Activate</button>`:''}</td></tr>`).join('')}</table>`;document.querySelectorAll('[data-test-model]').forEach(b=>b.addEventListener('click',async()=>{const model=models.find(x=>x.id===b.dataset.testModel),inputs={};for(const f of model.factors){const value=prompt(`${f.label} (${f.min}–${f.max})`);if(value!==null&&value!=='')inputs[f.code]=Number(value);}try{const result=await api(`/admin/qualification-models/${model.id}/test`,{method:'POST',body:{inputs}});alert(`Result: ${result.temperature} · score ${result.score}\n${result.contributions.map(x=>`${x.label}: ${Number(x.contribution).toFixed(2)}`).join('\n')}`);}catch(err){toast(err.message);}}));document.querySelectorAll('[data-approve-model]').forEach(b=>b.addEventListener('click',async()=>{const reason=prompt('Approval reason');if(!reason)return;try{await api(`/admin/qualification-models/${b.dataset.approveModel}/approve`,{method:'POST',body:{reason}});loadQualificationModels();}catch(err){toast(err.message);}}));document.querySelectorAll('[data-activate-model]').forEach(b=>b.addEventListener('click',async()=>{try{await api(`/admin/qualification-models/${b.dataset.activateModel}/activate`,{method:'POST'});loadQualificationModels();}catch(err){toast(err.message);}}));}catch(err){$('#qualification-model-table').textContent=err.message;}}
...
async function loadBrokers() {
  try {
    ...
    $('#broker-table').innerHTML = `<table><tr><th>User</th><th>Classification</th><th>Effective roles</th><th>Primary role</th><th>Team</th><th>Status</th><th>Actions</th></tr>
    ${brokers.map(b => `<tr>
