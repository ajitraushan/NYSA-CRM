...
async function renderCrmDashboard(selected={}){
  ...
  panels+=`<div class="dashboard-panel"><h3>Qualification and SLA</h3><div class="kv-grid"><div><b>Hot</b>${data.qualification.hot}</div><div><b>Warm</b>${data.qualification.warm}</div><div><b>Calls</b>${data.calls.total} · ${data.calls.durationSeconds}s</div><div><b>Call follow-up</b>${data.calls.followUpRequired}</div></div></div><div class="dashboard-panel"><h3>Inventory readiness</h3><div class="kv-grid"><div><b>Available</b>${data.inventory.available}</div><div><b>Stale</b>${data.inventory.stale}</div><div><b>Media not ready</b>${data.inventory.mediaNotReady}</div></div></div><div class="dashboard-panel"><h3>Recent customer activity</h3>${data.recentActivities.map(a=>`<button class="recent-row" data-dashboard-lead="${a.leadId}"><span class="recent-main"><b>${esc(a.subject)}</b><small>${esc(a.contactName)} · ${esc(a.activityType)} · ${fmtDate(a.createdAt)}</small></span></button>`).join('')||'<div class="empty">No recent activity</div>'}</div>`;
...
async function openLeadDocuments(lead){const o=overlay(`<div class="modal"><button class="close-x">×</button><h2>Private lead document upload</h2><p class="tool-note">PDF, DOCX, XLSX, JPG, PNG, WEBP and TXT only. Downloads require authenticated record access and are audited.</p><form id="document-form" class="form-grid"><div><label>Type *</label><input name="documentType" required placeholder="Offer Letter"></div><div><label>Title *</label><input name="title" required></div><div><label>Direction</label><select name="direction"><option>Inbound</option><option>Outbound</option><option>Internal</option></select></div><div><label>Classification</label><select name="classification"><option>private</option><option>restricted</option><option>internal</option></select></div><div><label>Recipient</label><input name="recipient"></div><div><label>File *</label><input name="file" type="file" required></div><button class="btn btn-primary btn-sm">Upload immutable version</button></form><div id="document-result"></div></div>`);$('#document-form',o).addEventListener('submit',async e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.target)),file=e.target.elements.file.files[0];try{const result=await api('/crm/documents',{method:'POST',body:{...f,file:undefined,leadId:lead.id,contactId:lead.contactId,fileName:file.name,mediaType:file.type,base64:await fileAsBase64(file)}});$('#document-result',o).innerHTML=`<p>Uploaded ${esc(result.version.fileName)} · SHA-256 <span class="mono">${esc(result.version.fileHash)}</span></p><a class="btn btn-sm" href="/api/crm/document-versions/${result.version.id}/download">Authenticated download</a>`;}catch(err){toast(err.message);}});}
...
async function renderAdmin() {
  ...
  loadOrganization();loadValueSets();loadSlaPolicies();loadRoutingRules();loadIntakeEvents();loadQualificationModels();loadAssumptionVersions();loadProposalTemplates();loadTeams(); loadInvites(); loadBrokers(); loadAudit();
}
...
async function createProposalTemplate(e){e.preventDefault();const f=Object.fromEntries(new FormData(e.target));try{await api('/admin/proposal-templates',{method:'POST',body:{...f,configuration:{brand:'NYSA Realty'}}});e.target.reset();loadProposalTemplates();}catch(err){toast(err.message);}}
