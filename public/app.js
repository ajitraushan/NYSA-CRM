...
async function createAssumptionVersion(e){e.preventDefault();const f=Object.fromEntries(new FormData(e.target)),id=f.id,items=collectFeeItems();delete f.id;try{await api(id?`/admin/regulatory-assumptions/${id}`:'/admin/regulatory-assumptions',{method:id?'PATCH':'POST',body:{...f,items}});toast(id?'Fee rule-set draft saved':'Regulatory and fee rule-set draft created');fillAssumptionForm();loadAssumptionVersions();}catch(err){toast(err.message);}}
