...
async function renderAdmin() {
  ...
  <div class="admin-section"><h2>NYSA Proposal Template Designer</h2><p class="tool-note">Choose curated business sources; do not enter database paths. Mandatory system data must be corrected in its authoritative record.</p><form id="proposal-template-form" class="form-grid"><div><label>Proposal type</label><select name="templateType"><option>Quick</option><option>Investment</option><option>Comparison</option></select></div><div><label>Template name *</label><input name="name" required></div><div><label>Brand version *</label><input name="brandVersion" required placeholder="NYSA-2026.1"></div><div class="span3"><label>Approved disclaimers (one per line)</label><textarea name="approvedDisclaimers"></textarea></div><div class="span3"><label>Ordered proposal sections</label><div id="proposal-sections"></div><button class="btn btn-sm" id="add-proposal-section" type="button">Add section</button></div><button class="btn btn-primary btn-sm">Create draft</button></form><div id="proposal-template-table">Loading...</div></div>
  ...
  $('#routing-defaults').addEventListener('click',configureDubaiRoutingDefaults);$('#add-qualification-factor').addEventListener('click',()=>addQualificationFactor());$('#add-fee-item').addEventListener('click',()=>addFeeItem());$('#assumption-new').addEventListener('click',()=>fillAssumptionForm());$('#assumption-clear').addEventListener('click',closeAssumptionForm);$('#add-proposal-section').addEventListener('click',()=>addProposalSection());$('#add-user-form').addEventListener('submit',addUser);
  addQualificationFactor({code:'budget_readiness',label:'Budget readiness',description:'Agent-confirmed budget preparedness',inputSource:'agent_confirmed',min:0,max:10,weight:100,required:true,missingTreatment:'reject'});addFeeItem({code:'dld_transfer_fee',label:'DLD transfer fee',calculationType:'percentage',calculationBasis:'purchase_price',ratePercent:4,transactionType:'purchase',payer:'contractual'});addProposalSection({code:'customer_name',label:'Customer name',source:'system',field:'contact.full_name',mandatory:true});addProposalSection({code:'properties',label:'Properties',source:'properties',mandatory:true});addProposalSection({code:'highlights',label:'Highlights',source:'agent_input',mandatory:true});loadDashboardTargetSetup();
