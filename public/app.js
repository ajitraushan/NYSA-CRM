...
async function saveOrganization(e){e.preventDefault();const form=e.target,body=Object.fromEntries(new FormData(form)),id=body.id,file=form.elements.logoFile.files[0];delete body.id;delete body.logoFile;try{const saved=await api(id?`/admin/organization-settings/${id}`:'/admin/organization-settings',{method:id?'PATCH':'POST',body});if(file)await api(`/admin/organization-settings/${saved.id}/logo`,{method:'POST',body:{fileName:file.name,mediaType:file.type,base64:await fileAsBase64(file)}});const message=id?`Company profile draft v${saved.version} updated`:`Company profile draft v${saved.version} created`;toast(message);fillOrganizationForm({});await loadOrganization();organizationFeedback(`${message}. It is listed below and remains inactive until approved and activated.`,'success');}catch(err){toast(err.message);organizationFeedback(`Draft not saved: ${err.message}`,'error');}}
