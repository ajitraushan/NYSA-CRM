...
function fillAssumptionForm(v=null){const form=$('#assumption-form');form.reset();form.elements.id.value=v?.id||'';form.elements.name.value=v?.name||'Dubai property transaction costs';form.elements.currency.value=v?.currency||'AED';form.elements.sourceReference.value=v?.assumptions?.sourceReference||'';form.elements.disclaimer.value=v?.disclaimer||'Illustrative estimate only; regulatory, service-provider, lender and property-specific charges may differ and are subject to verification before transaction.';$('#fee-items').innerHTML='';for(const item of v?.assumptions?.items||[{code:'dld_transfer_fee',label:'DLD transfer fee',calculationType:'percentage',calculationBasis:'purchase_price',ratePercent:4,transactionType:'purchase',payer:'contractual'}])addFeeItem(item);$('#assumption-save').textContent=v?`Save draft version ${v.version}`:'Create draft rule set';form.scrollIntoView({behavior:'smooth',block:'start'});}
function feeRuleSummary(x={}){const formula=String(x.calculationType||'').replaceAll('_',' '),basis=String(x.calculationBasis||'none').replaceAll('_',' '),amount=x.calculationType==='percentage_plus_fixed'?`${x.ratePercent||0}% + ${x.fixedAddition||0}`:x.calculationType==='conditional_fixed'?(x.bands||[]).map(b=>`${String(b.dimension).replaceAll('_',' ')} ${String(b.operator).replaceAll('_',' ')} ${b.value}: ${b.amount}`).join('; '):x.calculationType==='quantity'?`${x.unitAmount} per ${x.quantityCode||'unit'}`:x.calculationType==='estimate_range'?`${x.minAmount}–${x.maxAmount}`:x.calculationType==='fixed'?`${x.amount}`:x.ratePercent!==undefined?`${x.ratePercent}%`:'amount not set';return`${formula} · ${amount||'amount not set'} · on ${basis} · VAT ${x.vatPercent||0}% · ${String(x.transactionType||'all').replaceAll('_',' ')} · ${String(x.propertyType||'all').replaceAll('_',' ')}`;}
