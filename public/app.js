...
async function renderAdmin() {
  ...
  <div class="admin-section"><h2>Dashboard targets, benchmarks and exception thresholds</h2><form id="dashboard-target-form" class="admin-toolbar"><div><label>Metric</label><select name="metricCode"><option value="new_leads">New leads</option><option value="won_leads">Won leads</option><option value="hot_leads">Hot leads</option><option value="sla_breaches">SLA breaches</option><option value="sla_risk">SLA risk</option><option value="no_next_action">No next action</option><option value="stale_risk">Stale-risk leads</option><option value="team_capacity_pressure">Team-capacity pressure</option><option value="proposal_workload">Proposal workload</option><option value="customer_engagement">Customer engagement</option><option value="inventory_available">Available inventory</option><option value="inventory_stale">Aging inventory</option><option value="inventory_readiness_exposure">Inventory readiness exposure</option><option value="operational_exceptions">Operational exceptions</option></select></div><div><label>Scope</label><select name="scopeType"><option value="company">Company</option><option value="business_line">Business line</option><option value="team">Team</option><option value="agent">Agent</option></select></div><div><label>Scope ID</label><input name="scopeId"></div><div><label>Period start</label><input name="periodStart" type="date" required></div><div><label>Period end</label><input name="periodEnd" type="date" required></div><div><label>Target</label><input name="targetValue" type="number" step="0.01" required></div><div><label>Exception threshold</label><input name="exceptionThreshold" type="number" step="0.01"></div><div><label>Threshold rule</label><select name="thresholdDirection"><option value="high_bad">Above is exception</option><option value="low_bad">Below is exception</option></select></div><div><label>Unit</label><input name="unit" value="leads" required></div><div><label>Benchmark source</label><input name="benchmarkSource" placeholder="Board-approved plan"></div><div><label>Approved definition</label><input name="definition" required></div><button class="btn btn-primary btn-sm">Set target</button></form></div>
