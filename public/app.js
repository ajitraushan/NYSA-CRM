...
async function renderAdmin() {
  ...
  <div class="admin-section"><h2>Regulatory and fee assumption versions</h2><p class="tool-note">Maintain each approved calculation rule individually; proposals use calculated placeholders such as fee.dld_transfer_fee.</p><form id="assumption-form" class="form-grid"><div><label>Version name *</label><input name="name" value="UAE property finance" required></div><div><label>Currency</label><select name="currency"><option>AED</option><option>USD</option></select></div><div><label>Source/reference</label><input name="sourceReference" placeholder="Approved policy or authority reference"></div><div class="span3"><label>Regulatory and fee items</label><div id="fee-items"></div><button class="btn btn-sm" id="add-fee-item" type="button">Add fee item</button></div><div class="span3"><label>Estimate disclaimer *</label><textarea name="disclaimer" required>Illustrative estimate only; lender, regulatory, fee and property outcomes may differ and are not guaranteed.</textarea></div><button class="btn btn-primary btn-sm">Create draft version</button></form><div id="assumption-table">Loading...</div></div>
  ...
  $('#routing-defaults').addEventListener('click',configureDubaiRoutingDefaults);$('#add-qualification-factor').addEventListener('click',()=>addQualificationFactor());$('#add-fee-item').addEventListener('click',()=>addFeeItem());$('#add-proposal-section').addEventListener('click',()=>addProposalSection());$('#add-user-form').addEventListener('submit',addUser);
  addQualificationFactor({code:'budget_readiness',label:'Budget readiness',description:'Agent-confirmed budget preparedness',inputSource:'agent_confirmed',min:0,max:10,weight:100,required:true,missingTreatment:'reject'});addFeeItem({code:'dld_transfer_fee',label:'DLD transfer fee',calculationType:'percentage',ratePercent:4});addProposalSection({code:'customer_name',label:'Customer name',source:'system',field:'contact.full_name',mandatory:true});addProposalSection({code:'properties',label:'Properties',source:'properties',mandatory:true});addProposalSection({code:'highlights',label:'Highlights',source:'agent_input',mandatory:true});loadDashboardTargetSetup();
...
function addFeeItem(f={}){const row=document.createElement('div');row.className='maintenance-row fee-item';row.innerHTML=`<input data-field="label" placeholder="Business label" value="${esc(f.label||'')}"><input data-field="code" placeholder="stable_code" pattern="[a-z][a-z0-9_]*" value="${esc(f.code||'')}"><select data-field="calculationType"><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option><option value="tiered">Tiered</option></select><input data-field="ratePercent" type="number" step="0.0001" placeholder="Rate %" value="${f.ratePercent??''}"><input data-field="amount" type="number" step="0.01" placeholder="Fixed amount" value="${f.amount??''}"><input data-field="applicability" placeholder="Applicability" value="${esc(f.applicability||'All applicable scenarios')}"><button class="btn btn-sm" type="button">Remove</button>`;row.querySelector('[data-field="calculationType"]').value=f.calculationType||'percentage';row.querySelector('button').onclick=()=>row.remove();$('#fee-items').append(row);}
async function createAssumptionVersion(e){e.preventDefault();const f=Object.fromEntries(new FormData(e.target)),items=collectRows('.fee-item').map(x=>({...x,tiers:x.calculationType==='tiered'?[{from:0,to:null,ratePercent:x.ratePercent}]:undefined}));try{await api('/admin/regulatory-assumptions',{method:'POST',body:{...f,items}});toast('Regulatory and fee draft created');loadAssumptionVersions();}catch(err){toast(err.message);}}
async function loadAssumptionVersions(){try{const{versions}=await api('/admin/regulatory-assumptions');$('#assumption-table').innerHTML=`<table><tr><th>Name</th><th>Version</th><th>Items / placeholders</th><th>Effective</th><th>Status</th><th>Actions</th></tr>${versions.map(v=>`<tr><td>${esc(v.name)}</td><td>${v.version}</td><td>${(v.assumptions?.items||[]).length}<small>${esc((v.assumptions?.placeholders||[]).join(', '))}</small></td><td>${fmtDate(v.effectiveFrom)}</td><td>${esc(v.status)}</td><td>${v.status==='draft'?`<button class="btn btn-sm" data-approve-assumption="${v.id}">Approve</button> <button class="btn btn-sm" data-test-assumption="${v.id}">Test</button>`:''}${v.status==='approved'?`<button class="btn btn-primary btn-sm" data-activate-assumption="${v.id}">Activate</button>`:''}</td></tr>`).join('')}</table>`;document.querySelectorAll('[data-approve-assumption]').forEach(b=>b.addEventListener('click',async()=>{const reason=prompt('Approval reason');if(!reason)return;try{await api(`/admin/regulatory-assumptions/${b.dataset.approveAssumption}/approve`,{method:'POST',body:{reason}});loadAssumptionVersions();}catch(err){toast(err.message);}}));document.querySelectorAll('[data-test-assumption]').forEach(b=>b.addEventListener('click',async()=>{const baseAmount=Number(prompt('Test base amount','1000000'));if(!Number.isFinite(baseAmount))return;try{const result=await api(`/admin/regulatory-assumptions/${b.dataset.testAssumption}/test`,{method:'POST',body:{baseAmount}});alert(`Calculated fees: ${JSON.stringify(result.values)}\nTotal: ${result.total}`);}catch(err){toast(err.message);}}));document.querySelectorAll('[data-activate-assumption]').forEach(b=>b.addEventListener('click',async()=>{try{await api(`/admin/regulatory-assumptions/${b.dataset.activateAssumption}/activate`,{method:'POST'});loadAssumptionVersions();}catch(err){toast(err.message);}}));}catch(err){$('#assumption-table').textContent=err.message;}}
