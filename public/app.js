...
function previewProposalTemplate(){const c=proposalConfigurationFromForm(),fields=c.buyerBooklet.propertyFields.map(f=>PROPOSAL_PROPERTY_FIELDS.find(x=>x[0]===f.code)?.[1]).filter(Boolean),organization=organizationVersions.find(x=>x.status==='active'),logo=organization?.logo?.downloadUrl;overlay(`<div class="modal proposal-preview"><button class="close-x">×</button><header class="proposal-preview-brand">${logo?`<img src="${esc(logo)}" alt="${esc(organization.displayName)} approved logo">`:'<div class="proposal-logo-missing">No active approved logo</div>'}<div><b>${esc(organization?.displayName||'NYSA Realty')}</b><small>Quick Buyer Booklet · Brand ${esc(organization?.brandVersion||$('#proposal-template-form').elements.brandVersion.value||'not selected')}</small></div></header><h2>Sample buyer booklet preview</h2><p class="tool-note">Preview only — sample values are not saved.${organization?' The active company profile and approved logo shown here will govern live proposals.':' Activate an approved NYSA company profile to preview and generate with its governed logo.'}</p><h3>Your requirements</h3><p>Dubai Marina · Apartment · 2 bedrooms · AED 2.0m–2.5m · ${c.buyerBooklet.requireAvailabilityCheck?'Availability must be reconfirmed':'Availability check optional'}</p><h3>Matched shortlist (maximum ${c.buyerBooklet.maxProperties})</h3><article class="proposal-sample"><b>Sample Residence · Dubai Marina</b><p>${fields.join(' · ')}</p><small>Why it matches: preferred location and budget. Trade-off: sample text to be completed from verified inventory and agent review.</small></article><h3>Approved property media</h3><p>Up to ${c.buyerBooklet.maxMediaPerProperty} approved images per property.</p><h3>Indicative purchase timeline</h3>${c.buyerBooklet.timelineStages.map(x=>`<p><b>${esc(x.label)}</b><br><small>${esc(x.guidance)}</small></p>`).join('')}<h3>Next steps</h3><p>Confirm availability, arrange viewing and verify finance or readiness before commitment.</p><footer class="proposal-preview-footer">${esc(organization?.proposalFooter||'NYSA Realty | Private customer proposal')}</footer></div>`);}
