function dealWorkspaceHTML({deals=[],bookings=[],writable}){
  ...
  const approvalAction=['draft','completion_in_progress'].includes(deal.status)&&approvalReady?
    closureAuthority?`<form id="deal-approval-form" class="form-grid opportunity-inline-form">
      <div><label>Decision *</label><select name="decision" required><option value="approved">Approve Deal for authoritative closure</option><option value="returned">Return for correction</option><option value="rejected">Reject closure request</option></select></div>
