function dealWorkspaceHTML({deals=[],bookings=[],writable}){
  ...
  const parties=(deal.parties||[]).filter(x=>!x.effectiveTo),items=deal.checklistItems||[],gates=deal.closureGates||[],
    ...
    partyAdditionAllowed=dealEditable,
  ...
  const approvalAction=['draft','completion_in_progress'].includes(deal.status)&&approvalReady?
    closureAuthority?`<form id="deal-approval-form" class="form-grid opportunity-inline-form">
      <div><label>Decision *</label><select name="decision" required><option value="approved">Approve Deal for authoritative closure</option><option value="returned">Return for correction</option></select><small>Return keeps the Deal open for correction. Use the governed Close Lost path only when the transaction has genuinely failed.</small></div>
      <div class="span2"><label>Decision reason *</label><input name="reason" minlength="5" required placeholder="Record the review decision and required action"></div>
      <div><label>Decision evidence reference *</label><input name="evidenceReference" required placeholder="Email, meeting or maintained record"></div>
      <button class="btn btn-primary span3">Record management decision</button>
  ...
  const relinkAction=activeBooking&&activeBooking.id!==deal.bookingId&&['draft','completion_in_progress','approved'].includes(deal.status)?`<details class="opportunity-inline-form" open><summary>Link the new reservation to this retained Deal</summary>
    <div class="deal-origin-summary"><b>${esc(activeBooking.bookingReference)} · ${esc(activeBooking.listingProject)}</b><span>New exact accepted Offer Revision ${esc(activeBooking.acceptedRevisionNumber)}. Deal ${esc(deal.dealReference)} remains unchanged.</span></div>
    <form id="deal-create-form" class="form-grid" data-booking-id="${esc(activeBooking.id)}">
      ${activeBooking.offerType==='commercial'?'<div><label>Commercial transaction *</label><select name="commercialMode"><option value="">Select sale or rental</option><option value="sale">Commercial sale</option><option value="rental">Commercial rental</option></select></div>':''}
      <div><label>Revised target completion *</label><input type="datetime-local" name="targetCompletionAt" required></div>
      <div class="span3 opportunity-safety-note"><b>Stable identifiers</b><span>This atomically changes the Deal’s authoritative Inventory, Offer revision and Booking linkage. The Opportunity ID and Deal ID remain the same; prior lineage remains immutable.</span></div>
      <button class="btn btn-primary span3">Link reservation and retain Deal ID</button>
    </form></details>`:'';
  return `<div class="comments deal-workspace" data-flow-pane="deal" data-deal-id="${esc(deal.id)}" data-deal-version="${esc(deal.version)}">
    ...
    ${deal.bookingId&&['draft','completion_in_progress','approved'].includes(deal.status)?'<div class="deal-gate blocked"><b>Current accepted Offer and reservation are locked</b><span>Release or expiry must be recorded through the Booking. The Deal and Opportunity IDs remain stable; a replacement Inventory assignment must create a new Offer and Booking before becoming current.</span></div>':''}
    ${!deal.bookingId&&['draft','completion_in_progress','approved'].includes(deal.status)?'<div class="deal-gate blocked"><b>Deal retained — replacement lineage pending</b><span>The previous reservation has ended. Assign Inventory, create and accept a new Offer, reserve it, then link that Booking here without creating a new Deal ID.</span></div>':''}
    <div class="deal-head"><div><span>${esc(deal.dealReference)} · ${esc(deal.dealType.replaceAll('_',' '))}</span><h4>${esc(deal.listingProject||'No current Inventory linkage')}</h4><small>${deal.bookingReference?`Booking ${esc(deal.bookingReference)} · exact Offer Revision ${esc(deal.acceptedRevisionNumber)}`:'Awaiting a new accepted Offer and Booking'}</small></div><div><b>${fmtPrice(deal.agreedValue,deal.currency)}</b><span>${esc(deal.status.replaceAll('_',' '))}</span></div></div>
    ...
    <section><h4>Transaction parties</h4>${partyRows||'<div class="empty compact">No parties recorded.</div>'}
      ${partyAdditionAllowed?`<details class="opportunity-inline-form"><summary>${requiredPartiesComplete?'Add another transaction party':'Add missing required transaction party'}</summary><form id="deal-party-form" class="form-grid">
        <div><label>Party source *</label><select name="partyKind"><option value="contact">Customer / contact</option><option value="company">Company</option><option value="counterparty">Transaction-only counterparty</option></select></div>
        <div class="span2"><label>Maintained party *</label><select name="partyId" required><option value="">Loading permitted records…</option></select></div>
    ...
    ${relinkAction}<section><h4>${esc(deal.checklistName)} · Template version ${esc(deal.templateVersionNo)}</h4><p class="tool-note">This checklist is an exact instance of the approved template version; later template changes do not rewrite this Deal.</p>${itemRows}</section>${window.documentComplianceWorkspaceHTML?.({deal,writable})||''}${window.dealCommissionWorkspaceHTML?.({deal,writable})||''}${window.officialDocumentWorkspaceHTML?.({deal})||''}${lostAction}
  </div>`;
}
...
function bindDealWorkspace(root,{opportunityId,onChanged,writable=false}){
  ...
  const officialDeal=root.querySelector('.deal-workspace[data-deal-id]');if(officialDeal)window.bindOfficialDocumentWorkspace?.(root,{dealId:officialDeal.dataset.dealId,writable:true});
  ...
  if(partyForm&&dealRoot){let options={contacts:[],companies:[],counterparties:[]};const select=partyForm.elements.partyId,render=()=>{const kind=partyForm.elements.partyKind.value,rows=kind==='contact'?options.contacts:kind==='company'?options.companies:options.counterparties;select.innerHTML='<option value="">Select a maintained record</option>'+rows.map(x=>`<option value="${esc(x.id)}">${esc(x.fullName||x.name||x.displayName)}${x.email?' · '+esc(x.email):''}</option>`).join('');};
  ...
  approvalForm?.addEventListener('submit',async event=>{event.preventDefault();const body=Object.fromEntries(new FormData(approvalForm));try{await api(`/crm/deals/${dealRoot.dataset.dealId}/approval`,{method:'POST',body:{...body,expectedVersion:Number(dealRoot.dataset.dealVersion)}});refresh(body.decision==='approved'?'Opportunity completion approved for authoritative closure':'Opportunity completion returned for correction');}catch(error){toast(error.message,9000);}});
