...
export function buildMatchEvidence(requirement,listing){
  ...
  compare('Inventory lifecycle','Active Inventory',effectiveStatus,effectiveStatus!==undefined&&effectiveStatus!==null&&!['Closed','Sold','Rented'].includes(effectiveStatus));
...
export function rankInventoryMatches(requirement,listings=[]){
  ...
  return listings.filter(x=>x.effectiveStatus&&!['Closed','Sold','Rented'].includes(x.effectiveStatus)).map(listing=>{
    const criteria=[];
