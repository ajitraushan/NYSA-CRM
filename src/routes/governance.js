...
r.get('/admin/property-media-approval-policy',async(req,res)=>{
  ...
  const policy=await one('SELECT * FROM property_media_approval_policy LIMIT 1');
  res.json({policy});
});

r.patch('/admin/property-media-approval-policy',async(req,res)=>{
  if(!adminOnly(req,res))return;
  ...
  const prior=await one('SELECT * FROM property_media_approval_policy LIMIT 1');
  const policy=await transaction(async client=>{
    const updated=await one('UPDATE property_media_approval_policy SET manager_approval_required=$1,change_reason=$2,updated_by=$3,updated_at=NOW() RETURNING *',[req.body.managerApprovalRequired,reason,req.broker.id],client);
    await audit('PropertyMediaApprovalPolicy',updated.id,'policy_changed',req.broker.id,{from:prior?.managerApprovalRequired??true,to:updated.managerApprovalRequired,reason,appliesTo:'future_uploads'},client);
    return updated;
  });
  res.json({policy});
});
