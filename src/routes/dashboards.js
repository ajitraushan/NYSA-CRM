...
r.get('/crm/dashboard',async(req,res)=>{
  ...
  const [current,previous,stages,sources,teams,agents,trend,tasks,exceptions,proposals,calls,inventory,targets]=await Promise.all([
    one(`SELECT COUNT(*)::int AS leads,COUNT(*) FILTER(WHERE temperature='Hot')::int AS hot,COUNT(*) FILTER(WHERE temperature='Warm')::int AS warm,
      COUNT(*) FILTER(WHERE stage='Won')::int AS won,COUNT(*) FILTER(WHERE accepted_at IS NULL AND original_acceptance_due_at<NOW())::int AS acceptance_breaches,
    ...
    one(`SELECT COUNT(*) FILTER(WHERE (c.email IS NULL AND c.phone IS NULL))::int AS missing_contact,
      ...
      COUNT(*) FILTER(WHERE c.do_not_contact=0 AND NOT EXISTS(SELECT 1 FROM marketing_agreements ma WHERE ma.contact_id=c.id AND ma.status='executed' AND ma.effective_at<=NOW() AND (ma.expires_at IS NULL OR ma.expires_at>NOW()))
        AND NOT EXISTS(SELECT 1 FROM consent_evidence ce WHERE ce.contact_id=c.id AND ce.status='granted' AND ce.superseded_at IS NULL))::int AS missing_consent,
      ...
      COUNT(*) FILTER(WHERE EXISTS(SELECT 1 FROM website_intake_events w WHERE w.lead_id=l.id AND w.status='failed'))::int AS integration_failures
  ...
  res.json({dashboardType:type,view:type==='executive'?(req.query.view||'Executive'):type,dataAsOf:new Date(),lastRefresh:new Date(),filters:f.selected,period:{current:{from:f.start,to:f.end},prior:{from:prior.start,to:prior.end}},calculationContext:'Record-scoped operational data; reassignment history is not rewritten. Counts use distinct lead records.',kpis,qualification:{hot:current.hot,warm:current.warm},stages,sources,teams,agents,trend,tasks,exceptions,proposals,calls,inventory,recentActivities,
    futureMetrics:[{label:'Weighted opportunity pipeline',status:'unavailable',availableFrom:'Future opportunity/deal release'},{label:'Revenue and commission forecast',status:'unavailable',availableFrom:'Future transaction/commission release'}]});
});
