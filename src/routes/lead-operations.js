...
r.post('/admin/areas/import/preview',async(req,res)=>{if(!operationalAdmin(req))return res.status(403).json({error:'Administrator or Admin Assistant access required'});const file=decodeAndValidateFile({base64:req.body?.base64,mediaType:req.body?.mediaType,fileName:req.body?.fileName,maxBytes:2097152,allowedTypes:['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet']});if(file.error)return res.status(400).json({error:file.error});try{const sourceRows=await parseAreaWorkbook(file.buffer),existing=await many('SELECT stable_code,business_label FROM areas'),rows=validateAreaImportRows(sourceRows,{existingCodes:existing.map(x=>x.stableCode),existingLabels:existing.map(x=>x.businessLabel)});res.json({fileName:file.fileName,fileHash:file.fileHash,rowCount:rows.length,valid:rows.every(row=>!row.errors.length),rows});}catch(error){res.status(400).json({error:`Workbook could not be reviewed: ${error.message}`});}});
r.post('/admin/areas/import/commit',async(req,res)=>{if(!operationalAdmin(req))return res.status(403).json({error:'Administrator or Admin Assistant access required'});const reason=text(req.body?.reason),sourceRows=Array.isArray(req.body?.rows)?req.body.rows:[];if(!reason)return res.status(400).json({error:'An import reason is required'});if(!sourceRows.length||sourceRows.length>500)return res.status(400).json({error:'Provide between 1 and 500 reviewed area rows'});try{const imported=await transaction(async client=>{const existing=await many('SELECT stable_code,business_label FROM areas FOR SHARE',[],client),rows=validateAreaImportRows(sourceRows,{existingCodes:existing.map(x=>x.stableCode),existingLabels:existing.map(x=>x.businessLabel)}),invalid=rows.filter(row=>row.errors.length);if(invalid.length){const error=new Error('The reviewed import is no longer valid; preview it again');error.statusCode=409;error.rows=rows;throw error;}const created=[];for(const row of rows){const id=uuid(),area=await one('INSERT INTO areas(id,stable_code,business_label,emirate,display_order,created_by) VALUES($1,$2,$3,$4,$5,$6) RETURNING *',[id,row.stableCode,row.businessLabel,row.emirate,row.displayOrder,req.broker.id],client);await audit('Area',id,'bulk_imported',req.broker.id,{reason,rowNumber:row.rowNumber},client);created.push(area);}return created;});res.status(201).json({importedCount:imported.length,areas:imported});}catch(error){if(error.statusCode)return res.status(error.statusCode).json({error:error.message,rows:error.rows});if(error.code==='23505')return res.status(409).json({error:'An area changed after preview. Review the workbook again before importing.'});throw error;}});
