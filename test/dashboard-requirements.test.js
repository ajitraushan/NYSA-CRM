...
test('Role dashboards show the maintained reporting structure at the top without widening access',()=>{
  ...
  assert.match(routes,/kind:'director',title:'My direct reports'/);
  assert.match(routes,/kind:'manager',title:'My reporting agents'/);
  assert.match(routes,/kind:'agent',title:'My reporting line'/);
  assert.match(routes,/b\.job_role IN \('sales_agent','listing_agent'\)/);
  assert.match(routes,/FROM brokers b LEFT JOIN teams t ON t\.id=b\.team_id LEFT JOIN brokers m ON m\.id=t\.manager_id/);
