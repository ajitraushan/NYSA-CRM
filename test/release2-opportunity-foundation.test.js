...
test('Release 2 opportunity migration is additive immutable and reconciliation-first',()=>{
  ...
  assert.equal(migrations.at(-1),'038_release2_opportunity_foundation.sql');
...
test('opportunity API requires qualification scope attribution and optimistic concurrency',()=>{
  ...
  assert.match(domain,/Release 2\.1/);
});
...
test('opportunity workspace is separate and states the Release 1.1 compatibility boundary',()=>{
  ...
  for(const contract of ['Opportunity pipeline','Release 1.1 remains unchanged','No lead stage or historical record is converted automatically','Create / review opportunities','Original attribution · immutable','Legacy lead review ledger','Automatic conversion:'])assert.match(ui,new RegExp(contract.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
