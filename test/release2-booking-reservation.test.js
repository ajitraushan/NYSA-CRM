...
test('booking creation requires typed amount dates refundable state and evidence',()=>{
  ...
  assert.equal(good.value.bookingAmount,10000);
  assert.equal(validateBookingCreate({...good.value,bookingAmount:'20k'}).value.bookingAmount,20000);
...
test('R2.3B migration and API make reservation explicit conflict-safe and auditable',()=>{
  ...
  for(const marker of ["/crm/offers/:offerId/bookings","/crm/bookings/:bookingId/status","FOR UPDATE OF b,li","current_inventory_status","status='Reserved'","accepted_offer_revision_id","Reservation evidence document is required","active_booking_reference","Inventory is already reserved under","R2.3B enables explicit booking and reservation"])assert.match(routes,new RegExp(marker));
  for(const marker of ['Booking and reservation','Create explicit reservation','Reservation evidence','Release reservation','Cancel reservation','Inventory is reserved explicitly','Exact accepted offer revision','Inventory already reserved','e.g. 20K or 20,000','installBusinessAmountInputs'])assert.match(ui,new RegExp(marker));
});
