...
test('R2.3B migration and API make reservation explicit conflict-safe and auditable',()=>{
  ...
  for(const marker of ["/crm/offers/:offerId/bookings","/crm/bookings/:bookingId/status","/crm/offers/:offerId/reservation-reconciliation","legacy_reservation_reconciled","No active or historical governed Booking record","FOR UPDATE OF b","current_inventory_status","status='Reserved'","accepted_offer_revision_id","Reservation evidence document is required","active_booking_reference","active_booking_opportunity_id","Property is already reserved under","Only the maintained manager for this Opportunity","R2.5 reconciles the accepted Release 2 flow"])assert.match(routes,new RegExp(marker));
