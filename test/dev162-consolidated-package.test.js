...
test('dev.162 candidate is checksum-bound, deterministic-scope and migration-unique',()=>{
  const zip=fs.readFileSync(artifact('nysa-core-consolidated-crm-test-dev162.zip'));
  const sha=crypto.createHash('sha256').update(zip).digest('hex');
  const checksum=fs.readFileSync(artifact('nysa-core-consolidated-crm-test-dev162.sha256.txt'),'utf8').trim();
  const manifest=JSON.parse(fs.readFileSync(artifact('nysa-core-consolidated-crm-test-dev162.manifest.json'),'utf8'));
  assert.equal(checksum,`${sha}  nysa-core-consolidated-crm-test-dev162.zip`);
  assert.equal(manifest.packageSha256,sha);
  assert.equal(manifest.version,'2.1.0-dev.162');
  ...
  assert.deepEqual(manifest.verification.ordinary,{total:1199,passed:1172,failed:0,skippedProtected:27});
  ...
  assert.deepEqual(manifest.verification.uat080QuestionnaireWiring,{total:2,passed:2,failed:0});
