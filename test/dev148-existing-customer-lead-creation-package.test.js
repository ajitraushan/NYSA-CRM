...
const packageName='nysa-core-consolidated-crm-test-dev148.zip';
...
test('dev.148 package checksum identity and Lead-creation correction reconcile',()=>{
  ...
  assert.equal(manifest.version,'2.1.0-dev.148');
  assert.equal(manifest.baselineVersion,'2.1.0-dev.147');
  assert.equal(manifest.migrationCount,95);
  assert.equal(names.filter(name=>/^src\/migrations\/\d{3}_.+\.sql$/.test(name)).length,95);
  for(const required of ['app.cjs','package.json','package-lock.json','public/app.js','src/routes/crm.js','src/routes/website-intake.js','src/migrations/095_dev146_full_scope_uat_governance.sql','docs/CRM_TEST_DEV147_DEPLOYMENT_COMPLETION.md','docs/CRM_TEST_DEV148_EXISTING_CUSTOMER_LEAD_CREATION_HOTFIX.md','MANIFEST.sha256','RUNTIME_MANIFEST.sha256'])assert.ok(names.includes(required),required);
  ...
  const packagedCrm=Buffer.from(entries['src/routes/crm.js']).toString('utf8');
  assert.match(packagedCrm,/selfAssigned\?'assigned':'unassigned'/);
  assert.match(packagedCrm,/selfAssigned\?'accepted':'queued'/);
  const packagedWebsite=Buffer.from(entries['src/routes/website-intake.js']).toString('utf8');
  assert.match(packagedWebsite,/b\.requirement\?\.purpose==='investment'\?\['buyer','investor'\]/);
  assert.match(packagedWebsite,/rule\?\.teamId\|\|null,null,'unassigned'/);
  assert.match(packagedWebsite,/b\.profile\?'website_profile':'broker_recorded'/);
  assert.match(packagedWebsite,/versionNo=Number\(current\.versionNo\)\+1/);
...
test('dev.148 deployer is CRM Test locked and accepts only the dev.147 migration-095 baseline',()=>{
  const source=text('deploy-crm-test-consolidated-dev148.sh');
  for(const marker of ['EXPECTED_ROOT=/home/nysareal/nysa-core-dashboard-dd6262a-stage','EXPECTED_DATABASE=nysareal_nysa_r2_rehearsal','EXPECTED_VERSION=2.1.0-dev.148','PREVIOUS_VERSION=2.1.0-dev.147','BASELINE_MIGRATION=095_dev146_full_scope_uat_governance.sql','EXPECTED_PACKAGE=nysa-core-consolidated-crm-test-dev148.zip','expected exactly one live CRM Test worker','/api/health','/api/readiness','pre-dev148.dump','pre-dev148-app.tar.gz','Production snapshot changed','Production clone snapshot changed'])assert.ok(source.includes(marker),marker);
