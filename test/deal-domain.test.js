...
test('R2.4A migration, API and workspace preserve exact commercial evidence and closure gates',()=>{
  ...
  assert.match(ui,/managerReviewComplete=items\.some\(x=>x\.required&&x\.responsibleRole==='manager'&&x\.status==='completed'\)/);
  assert.match(ui,/partyAdditionAllowed=dealEditable&&!managerReviewComplete/);
