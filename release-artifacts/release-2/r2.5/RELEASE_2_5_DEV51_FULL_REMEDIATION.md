# Release 2.5 dev.51 — Full remediation acceptance
...
| R2.5-REG-CACHE-014 | Browser assets carry the dev.51 cache identity | Cache regression contract | PASS |
...
| R2.5-ASGN-ELIG-020 | Agent assignment queue contains only claimable recycled leads; claim eligibility consistently honors primary team, active membership and active role assignment | Assignment queue scope, UI and atomic-claim tests | PASS |
...
Deployment target: `2.0.0-dev.51`, migrations `051_release2_final_uat_remediation.sql` and `052_release2_full_remediation_foundation.sql`.
